defmodule HelloWeb.UserTracker do
  ...
  def init(_map) do
    ...
    Phoenix.PubSub.subscribe(Hello.PubSub, @channel_users_gossip)
  ...
  def handle_cast({:user_offline, user_handle}, %{ets_users: ets_users, ets_node_users: ets_node_users} = state) do
    ...
    for node <- Node.list() do
      Phoenix.PubSub.direct_broadcast(
        node, Hello.PubSub, @channel_users_gossip, %{event: "users_sync", node: Node.self(), users: _ets_user_list(ets_node_users, Node.self())}
      )
    end

    Phoenix.PubSub.local_broadcast(
      Hello.PubSub, @channel_users_local, %{event: "users_online", node: Node.self(), users: _ets_user_list(ets_node_users)}
    )
  ...
  def handle_info({:nodedown, node}, %{ets_node_users: ets_node_users} = state) do
    ...
    Phoenix.PubSub.broadcast(
      Hello.PubSub, @channel_clusters, %{event: "nodes_sync", nodes: nodes}
    )
  ...
  def handle_info(%{event: "users_sync", node: node, users: users} = _params, socket) do
    ...
    Phoenix.PubSub.local_broadcast(
      Hello.PubSub, @channel_users_local, %{event: "users_online", node: Node.self(), users: _ets_user_list(:node_users)}
    )
